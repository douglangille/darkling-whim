...
function buildAliasIndex() {
  ...
  for (const tf of app.vault.getMarkdownFiles()) {
    ...
    if (Array.isArray(aliases)) {
      for (const a of aliases) if (a && a.trim()) idx.set(a.trim(), tf);
    }
    if (typeof fm.title === "string" && fm.title.trim()) {
      idx.set(fm.title.trim(), tf);
    }
  }
  return idx;
}
function resolveTarget(raw, basePath, aliasIdx) {
  ...
  if (hash !== -1) {
    section = target.slice(hash + 1).trim();
...
module.exports = async function(tp) {
  ...
  if (!copied) {
    new Notice("Clipboard unavailable; copy manually.", 6000);
    return;
  }
  ...
  if (unresolved.length) {
    msg += ` • Unresolved: ${[...new Set(unresolved)].slice(0,5).join(", ")}${unresolved.length>5?"…":""}`;
  }
  new Notice(msg, 6000);

  // nothing returned, no insertion into note
};